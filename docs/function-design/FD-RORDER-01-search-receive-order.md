...
**Evidence**: `LEGACY_CONFIRMED` — `action/rorder/InputROrderAction.java:72-84` (URL constants)  
**Evidence**: `LEGACY_CONFIRMED` — `service/RoSlipService.java` (search pattern)
...
|----|---------|--------|------------|
| OQ-RORDER-01 | Search action dùng GET hay POST? Legacy có thể dùng POST (Struts form) | LOW | `INFERRED_FROM_CODE` |
| OQ-RORDER-02 | Có filter theo product code không? | LOW | `UNKNOWN` |
