...
**Evidence**: `LEGACY_CONFIRMED` — `action/sales/InputSalesAction.java:150-240`  
**Evidence**: `LEGACY_CONFIRMED` — `action/sales/InputSalesAction.java:79-115` (service injections)
...
**Evidence**: `LEGACY_CONFIRMED` — WF-04 Main Code Path, WRITE table list  
**Evidence**: `LEGACY_CONFIRMED` — `service/InputStockSalesService` injection trong `InputSalesAction.java:98`
...
**Evidence**: `LEGACY_CONFIRMED` — WF-04 Database Tables WRITE: "với 40+ denormalized snapshot columns"  
**Evidence**: `LEGACY_CONFIRMED` — `DB/sql/createtable/CREATE.sql:1755-1906`
...
**Evidence**: `LEGACY_CONFIRMED` — WF-04 Validation Rules #6
...
> `LEGACY_CONFIRMED`: `roLineId` là app-layer FK (không có DB FK constraint trong CREATE.sql).  
> `TARGET_DECISION`: EAD slip creation là side effect — implement trong same transaction.
...
|---|------|--------|------------|
| V1 | `customerCode` required, tồn tại, not deleted | WF-04 | `LEGACY_CONFIRMED` |
| V2 | `salesDate` required, valid date | WF-04 | `LEGACY_CONFIRMED` |
| V3 | Lines: `productCode` required, tồn tại | WF-04 | `LEGACY_CONFIRMED` |
| V4 | Lines: `quantity` > 0, `unitPrice` >= 0 | WF-04 | `LEGACY_CONFIRMED` |
| V5 | Lines: `rackCodeSrc` tồn tại nếu nhập | WF-04 | `LEGACY_CONFIRMED` |
| V6 | `billId` IS NULL → không cho sửa/xóa nếu đã chốt | WF-04 | `LEGACY_CONFIRMED` |
| V7 | Double-submit: idempotency key | WF-04 | `LEGACY_CONFIRMED` |
...
**Target**: `TARGET_DECISION` — tất cả 3 blocks trong 1 `prisma.$transaction()` để đảm bảo atomicity
...
|----|---------|--------|------------|
| OQ-SALES-01 | `InputStockSalesService.calcCost()` dùng FIFO hay LIFO? Hay average cost? | HIGH | `INFERRED_FROM_CODE` ("FIFO/LIFO inferred") |
| OQ-SALES-02 | Khi update SALES, EAD slip có được update/reverse không? | HIGH | `UNKNOWN` |
| OQ-SALES-03 | 40+ snapshot columns: cần liệt kê đầy đủ từ DDL trước khi implement | HIGH | `UNKNOWN` |
| OQ-SALES-04 | Partial fulfillment từ RO: `quantity` trong SALES < `quantity` trong RO line — có kiểm soát không? | HIGH | `UNKNOWN` |
