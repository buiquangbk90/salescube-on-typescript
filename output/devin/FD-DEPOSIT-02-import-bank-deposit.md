...
**Evidence**: `Confirmed by code` — WF-06 External Integrations "Import Sao Kê Ngân Hàng"  
**Evidence**: `Confirmed by code` — `action/deposit/ImportBankDepositAction.java` (all)
...
**Evidence**: `Confirmed by code` — WF-06 External Integrations "Import COD từ Shipper"  
**Evidence**: `Confirmed by code` — `action/deposit/ImportDeliveryDepositAction.java` (all)
...
**Evidence**: `Confirmed by code` — WF-06 Import flows
...
> `Target decision`: Staging tables (`BANK_DEPOSIT_WORK`, `DELIVERY_DEPOSIT_WORK`) có thể implement bằng in-memory processing + transaction thay vì persist vào DB — cần confirm.
...
|---|------|--------|------------|
| V1 | File type phải là CSV hoặc TSV | `AbstractXSVUploadAction` | `Confirmed by code` |
| V2 | SKU/product trong COD file phải match SALES_SLIP | WF-06 | `Confirmed by code` |
| V3 | Bank match: amount + date phải khớp với BILL_TRN | WF-06 | `Confirmed by code` |
| V4 | Unmatched rows → warning, không fail toàn bộ import | WF-06 | `Confirmed by code` |
...
|----|---------|--------|------------|
| OQ-DEPOSIT-05 | Bank file format: CSV hay TSV? Có schema cố định hay theo từng ngân hàng? | HIGH | `Unknown / needs verification` |
| OQ-DEPOSIT-06 | COD file format: shipper nào? Format có chuẩn không hay theo từng shipper? | HIGH | `Unknown / needs verification` |
| OQ-DEPOSIT-07 | Staging DB hay in-memory? Nếu DB thì cần Prisma model cho WORK tables | MEDIUM | `Unknown / needs verification` |
| OQ-DEPOSIT-08 | Manual review UI cho unmatched rows: có trong scope P1 không? | MEDIUM | `Unknown / needs verification` |
