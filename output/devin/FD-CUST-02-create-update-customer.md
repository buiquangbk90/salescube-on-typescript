...
**Evidence**: `Confirmed by code` — `service/CustomerService.java`, `entity/Customer.java`
...
**Evidence**: `Confirmed by code` — `service/CustomerHistoryService.java`
...
**Evidence**: `Confirmed by code` — `service/SeqMakerService.java` (pattern chung toàn hệ thống)
...
**Evidence**: `Confirmed by code` — WF-02 External Integrations section
...
> `Target decision`: `customerCode` là String PK (user-defined) — không dùng `@default(autoincrement())`.  
> `Target decision`: SEQ_MAKER trong legacy dùng cho internal ID phân biệt với `customerCode` — cần xác minh.  
> `Unknown / needs verification`: Chưa rõ `SEQ_MAKER` allocate field nào trong CUSTOMER_MST (PK hay internal seq?).
...
|---|------|--------|------------|
| V1 | `customerCode` required, max 15 chars, alphanumeric | WF-02 | `Confirmed by code` |
| V2 | `customerName` required, max 60 chars | WF-02 | `Confirmed by code` |
| V3 | `customerCode` phải unique (check trước INSERT) | `CustomerService.findByCode()` | `Confirmed by code` |
| V4 | `customerTel` max 15 chars | WF-02 | `Confirmed by code` |
| V5 | `customerEmail` format validation | WF-02 | `Confirmed by code` |
| V6 | `maxCreditLimit` numeric >= 0 | WF-02 | `Confirmed by code` |
| V7 | `cutoffGroup` phải có trong CATEGORY_TRN | WF-02 | `Confirmed by code` |
| V8 | `customerRankCategory` phải có trong CUSTOMER_RANK_MST | WF-02 | `Confirmed by code` |
...
**Evidence**: `Confirmed by code` — Seasar2 `@Transaction` interceptor pattern
...
|----|---------|--------|------------|
| OQ-CUST-02 | SEQ_MAKER allocate field nào trong CUSTOMER_MST? Không phải `CUSTOMER_CODE` (user-defined) | HIGH | `Unknown / needs verification` |
| OQ-CUST-03 | Cần ZIP_MST trong target DB không? Hay call external Japan Post API? | MEDIUM | `Unknown / needs verification` |
| OQ-CUST-04 | `CustomerRelService` (parent-child KH): có bao gồm trong scope P1 không? | MEDIUM | `Unknown / needs verification` |
| OQ-CUST-05 | Copy customer: copy toàn bộ 59 fields hay chỉ master fields? | LOW | `Unknown / needs verification` |
