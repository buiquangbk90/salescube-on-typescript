...
**Evidence**: `Confirmed by code` — `entity/Customer.java` (59 fields), `service/CustomerService.java`  
**Evidence**: `Confirmed by code` — `entity/sql/customer/` (21 SQL files)
...
**Evidence**: `Inferred from code` — từ entity fields `Customer.java`, SQL pattern của module tương tự
...
|---|------|--------|------------|
| V1 | Tất cả criteria là optional (search all nếu không nhập) | Convention Struts form | `Confirmed by code` |
| V2 | `DEL_DATETM IS NULL` mặc định (chỉ hiện active) | SQL pattern | `Confirmed by code` |
| V3 | Pagination: offset/limit | Struts pagination | `Inferred from code` |
...
|----|---------|--------|------------|
| OQ-CUST-01 | Sort order: luôn là CUSTOMER_CODE ASC hay user có thể chọn? | LOW | `Inferred from code` |
| OQ-CUST-02 | Page size: legacy dùng bao nhiêu records/page? | LOW | `Unknown / needs verification` |
