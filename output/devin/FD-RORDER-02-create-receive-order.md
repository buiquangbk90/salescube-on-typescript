...
**Evidence**: `Confirmed by code` — `action/rorder/InputROrderAction.java:1-100`  
**Evidence**: `Confirmed by code` — `service/RoSlipService.java`, `service/RoLineService.java`
...
**Evidence**: `Confirmed by code` — WF-03 Validation Rules #8, Status Transitions
...
**Evidence**: `Confirmed by code` — WF-03 Validation Rules #7
...
**Evidence**: `Confirmed by code` — WF-03 Validation Rules #6
...
**Evidence**: `Confirmed by code` — WF-03 Main Code Path
...
> `Target decision`: `roSlipId` là String PK được allocate bởi SEQ_MAKER (không dùng `@default(autoincrement())`).  
> `Confirmed by code`: `restQuantity` luôn = `quantity` khi insert — NOT NULL.  
> `Target decision`: Customer name snapshot vào `RO_SLIP_TRN` để không bị thay đổi khi CUSTOMER_MST update.
...
|---|------|--------|------------|
| V1 | `customerCode` required, phải tồn tại (DEL_DATETM IS NULL) | WF-03 | `Confirmed by code` |
| V2 | `roDate` required, valid date | WF-03 | `Confirmed by code` |
| V3 | Phải có ít nhất 1 line | Convention | `Inferred from code` |
| V4 | Mỗi line: `productCode` required, phải tồn tại | WF-03 | `Confirmed by code` |
| V5 | Mỗi line: `quantity` > 0 | WF-03 | `Confirmed by code` |
| V6 | Mỗi line: `unitPrice` >= 0 | WF-03 | `Confirmed by code` |
| V7 | Credit limit: warning nếu vượt, không block | WF-03 | `Confirmed by code` |
| V8 | Double-submit: idempotency key | WF-03 TokenProcessor | `Confirmed by code` |
| V9 | Stock: warning nếu không đủ, không block | WF-03 | `Confirmed by code` |
...
**Evidence**: `Confirmed by code` — Seasar2 `@Transaction` per service  
**Target**: `Target decision` — explicit `prisma.$transaction()` để đảm bảo atomicity
...
**Evidence**: `Confirmed by code` — WF-03 Main Code Path, `utility/DiscountUtil`

> `Unknown / needs verification`: Chưa biết cụ thể công thức discount (% hay fixed amount, theo qty brackets hay flat rate)
...
|----|---------|--------|------------|
| OQ-RORDER-03 | SEQ_MAKER cho RO_SLIP_TRN: format của RO_SLIP_ID là gì? Số nguyên hay string có prefix? | HIGH | `Unknown / needs verification` |
| OQ-RORDER-04 | DiscountUtil: discount theo % hay fixed amount? Có bracket (qty range) không? | HIGH | `Unknown / needs verification` |
| OQ-RORDER-05 | REST_QUANTITY được xử lý khi UPDATE (thay đổi quantity) như thế nào? | HIGH | `Unknown / needs verification` |
| OQ-RORDER-06 | Copy từ Estimate: có trong scope P1 không? | LOW | `Target decision` |
