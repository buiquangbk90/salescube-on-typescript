...
**Evidence**: `Confirmed by code` — `action/bill/CloseBillAction.java:99-141`  
**Evidence**: `Confirmed by code` — `service/BillService.java:47-80`
...
**Evidence**: `Confirmed by code` — `action/bill/CloseBillAction.java:150-196`
...
**Evidence**: `Confirmed by code` — WF-05 Validation Rules #2, `service/YmService`
...
**Evidence**: `Confirmed by code` — WF-05 Main Code Path, `service/BillAndArtService.java`
...
**Evidence**: `Confirmed by code` — WF-05 Error Handling: "Partial commit (timeout)"
...
> `Target decision`: Implement close bill trong **một transaction duy nhất per customer** (không phải một transaction cho tất cả KH) — giảm risk partial commit.
...
|---|------|--------|------------|
| V1 | `cutOffDate` required, format valid | WF-05 | `Confirmed by code` |
| V2 | `ymService.getYm(cutOffDate)` phải valid | WF-05 | `Confirmed by code` |
| V3 | Ít nhất 1 KH được chọn | WF-05 | `Confirmed by code` |
| V4 | Không chốt 2 lần cùng tháng cho cùng KH | WF-05 | `Confirmed by code` |
| V5 | `CUTOFF_GROUP` KH phải khớp với nhóm đang chốt | WF-05 | `Confirmed by code` |
| V6 | Reopen: kiểm tra trạng thái bill hợp lệ | WF-05 | `Confirmed by code` |
...
**Target**: `Target decision` — một transaction per customer để tránh partial commit toàn bộ batch.
...
|----|---------|--------|------------|
| OQ-BILL-01 | `ART_BALANCE_TRN`: có sử dụng trong target không? Cấu trúc bảng này là gì? | HIGH | `Unknown / needs verification` |
| OQ-BILL-02 | `BILL_TRN.STATUS` có các giá trị nào? WF chỉ thấy "0" | MEDIUM | `Unknown / needs verification` |
| OQ-BILL-03 | Reopen: CUSTOMER_MST.LAST_CUTOFF_DATE được set về ngày nào? (kỳ trước?) | HIGH | `Inferred from code` |
| OQ-BILL-04 | Invoice PDF (MakeOutBillAction) có trong scope P1 không? | MEDIUM | `Target decision` |
