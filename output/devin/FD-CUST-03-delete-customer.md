...
**Evidence**: `Confirmed by code` — WF-02 Status Transitions  
**Evidence**: `Confirmed by code` — `service/CustomerService.java`
...
**Evidence**: `Inferred from code` — WF-02 Validation Rules #9, pattern phổ biến trong hệ thống
...
**Evidence**: `Confirmed by code` — WF-02 Status Transitions diagram
...
|---|------|--------|------------|
| V1 | KH phải tồn tại (`DEL_DATETM IS NULL`) | `CustomerService` | `Confirmed by code` |
| V2 | Không xóa KH có `SALES_SLIP_TRN` chưa đóng | WF-02 V#9 | `Confirmed by code` |
...
|----|---------|--------|------------|
| OQ-CUST-06 | Check giao dịch: chỉ SALES_SLIP_TRN hay cả RO_SLIP_TRN, BILL_TRN, DEPOSIT_SLIP_TRN? | HIGH | `Inferred from code` |
| OQ-CUST-07 | Sau soft-delete, CUSTOMER_REL (parent-child) có bị cascade không? | MEDIUM | `Unknown / needs verification` |
