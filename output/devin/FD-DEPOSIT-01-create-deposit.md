...
**Evidence**: `Confirmed by code` — `action/deposit/InputDepositAction.java:1-60`  
**Evidence**: `Confirmed by code` — `action/deposit/InputDepositAction.java:64-100` (service injections)
...
**Evidence**: `Confirmed by code` — WF-06 Line detail `DEPOSIT_CATEGORY ∈ {...}`
...
**Evidence**: `Confirmed by code` — WF-06 Validation Rules #7
...
**Evidence**: `Confirmed by code` — WF-06 Status Transitions
...
**Evidence**: `Confirmed by code` — WF-06 Main Code Path
...
|---|------|--------|------------|
| V1 | `customerCode` required | WF-06 | `Confirmed by code` |
| V2 | `depositDate` required, valid date | WF-06 | `Confirmed by code` |
| V3 | Lines: `depositCategory` required, in enum | WF-06 | `Confirmed by code` |
| V4 | Lines: `price` > 0 | WF-06 | `Confirmed by code` |
| V5 | Lines: `bankId` required nếu category = bank | WF-06 | `Confirmed by code` |
| V6 | Lines: `instDate` required nếu category = installment | WF-06 | `Confirmed by code` |
| V7 | sum(lines.price) = totalAmount | WF-06 | `Confirmed by code` |
| V8 | Không xóa/sửa phiếu STATUS = `"1"` | WF-06 | `Confirmed by code` |
...
|----|---------|--------|------------|
| OQ-DEPOSIT-01 | `ART_BALANCE_TRN.ART_ID` link sau ART closing: qui trình ART closing là gì? Có trong P1 không? | HIGH | `Unknown / needs verification` |
| OQ-DEPOSIT-02 | DEPOSIT_CATEGORY enum: có thêm value nào ngoài danh sách không? (check CATEGORY_TRN) | MEDIUM | `Unknown / needs verification` |
| OQ-DEPOSIT-03 | Bank match fail khi import: cần manual review UI riêng không? | MEDIUM | `Unknown / needs verification` |
| OQ-DEPOSIT-04 | `setoff` category: bù trừ công nợ — cơ chế cụ thể là gì? Link với giao dịch nào? | HIGH | `Unknown / needs verification` |
